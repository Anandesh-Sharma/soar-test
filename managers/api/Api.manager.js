    ...
    async mw(req, res, next){
        ...
        let hotBolt = this.mwsExec.createBolt({stack: targetStack, req, res, onDone: async ({req, res, results})=>{
            ...
            if(result.selfHandleResponse){
                return this.managers.responseDispatcher.dispatch(res, {ok:true, data: result, code: statusCode});
                // do nothing if response handeled
            } else {
                
                if(result.errors){
                    return this.managers.responseDispatcher.dispatch(res, {ok: false, errors: result.errors, code: statusCode});
                } else if(result.error){
                    return this.managers.responseDispatcher.dispatch(res, {ok: false, message: result.error, code: statusCode});
                } else {
                    return this.managers.responseDispatcher.dispatch(res, {ok:true, data: result, code: statusCode});
                }
            }
